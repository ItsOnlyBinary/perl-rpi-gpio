...
sub new
{
	...
	if(ref $parameters{PIN} eq 'ARRAY') {
		...
		unless(!defined $_ || $_ =~ m/^\d+\z/) {
		    warn 'Unexpected PIN configuration';
		    $pass--;
		}
	    }
	    ...
	    warn "PIN needs to be array ref"
	}
    }
	...
	if(ref $parameters{BCM} eq 'ARRAY') {
		...
		unless(defined $_ && $_ =~ m/^\d+\z/) {
		    warn 'Unexpected BCM configuration';
		    $pass--;
		}
	    }
	    ...
	    warn "BCM needs to be array ref"
	}
    }
...
sub setup
{
    ...
    if(-e '/sys/class/gpio/gpio'.$channel) {
	open(my $unexport, '>', '/sys/class/gpio/unexport');
	print $unexport $channel;
	close($unexport);
    }
    ...
    #set gpio direction
    open(my $gpiodir, '>', '/sys/class/gpio/gpio'.$channel.'/direction');
    print $gpiodir $direction;
    close($gpiodir);
...
sub output
{
    ...
    open(my $gpio, '>', '/sys/class/gpio/gpio'.$channel.'/value');
    print $gpio $value;
    close($gpio);
...
sub input
{
    ...
    $channel = $self->validate($channel);
    if(!defined($channel)) {
	warn 'Invalid channel used for GPIO input';
	return undef;
    }
